 ...
 *
 * OAuth bounce (where Google redirects before returning to this site):
 *
 * | Where you open the site              | Google redirectTo                         |
 * |--------------------------------------|-------------------------------------------|
 * | localhost / 127.0.0.1 (:4000/:3000)  | http://localhost:54321/auth/callback      |
 * | LAN IP (192.168.x.x) + tunnel env    | {tunnel}/visitor-auth/callback/{pendingId}|
 ...
 *
 * Prod hosted coding (:3000) is built with basePath /t/{id}/p/{project}.
 * Use that prefix in URLs. EGDesk injects NEXT_PUBLIC_EGDESK_API_URL at build/start.
 * localhost and 127.0.0.1 on the same port count as the same site for session checks.
 *
