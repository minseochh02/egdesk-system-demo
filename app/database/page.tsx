...
export default function Home() {
  ...
  const [kakaoGuideHref, setKakaoGuideHref] = useState('/kakao-mcp');
  ...
  useEffect(() => {
    ...
    setKakaoGuideHref(`${basePath}/kakao-mcp`);
    setLandingHref(basePath || '/');
    fetchData();
  }, []);
  ...
  const handleInsertDemo = async () => {
    ...
    setInsertMsg(null);
    try {
      const res = await apiFetch('/api/create-row', { method: 'POST' });
      const json = await res.json();
    ...
    <main style={{ maxWidth: 960, margin: '0 auto', padding: '40px 24px' }}>
      ...
      <div style={{ marginBottom: 32 }}>
        <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap', marginBottom: 18 }}>
          ...
          <a href={kakaoGuideHref} style={navLinkStyle}>Kakao MCP guide</a>
        </div>
        <h1 style={{ fontSize: 28, fontWeight: 700, marginBottom: 8 }}>
          EGDesk Database Demo
        </h1>
      ...
      <div style={{
        ...
        <ol style={{ marginTop: 8, paddingLeft: 20 }}>
          ...
          <li><code>lib/api.ts</code> — provides <code>apiFetch()</code> so API calls work with tunnel basePath</li>
          <li>Your code uses <code>apiFetch('/api/...')</code> instead of <code>fetch()</code> — <strong>no connection strings, no credentials in code</strong></li>
        </ol>
      </div>
